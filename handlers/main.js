import { LocationAttachment, Keyboard } from '@maxhub/max-bot-api';
import { getAbiturientMenu } from '../keyboards/abiturient.js';
import { getUpcomingDeadlines } from '../config/deadlines.js'; // Добавляем этот импорт

const WELCOME_IMAGE_URL = 'https://static.tildacdn.com/tild6234-3964-4832-b930-393763383461/196A0054_1.jpg';


export function setupMainHandlers(bot) {
  // Стартовая команда
  bot.command('start', async (ctx) => {
    const upcoming = getUpcomingDeadlines();
    
    let deadlineInfo = '';
    if (upcoming.length > 0) {
      const nearest = upcoming[0];
      deadlineInfo = `\n\n⏰ Ближайший дедлайн: ${nearest.formattedDate} (через ${nearest.daysLeft} дней)`;
    }

    const message = 
      `Добро пожаловать в бота для абитуриентов ДВФУ! 🎓\n\n` +
      `Я помогу вам с поступлением:\n` +
      `• Отвечу на частые вопросы\n` +
      `• Расскажу о направлениях подготовки\n` +
      `• Помогу рассчитать шансы поступления\n` +
      `• Напомню о важных датах\n` + 
      `Конкурсные списки и детали поступления смотрите на официальном сайте ДВФУ https://postupi.dvfu.ru/`;



       try {
      // Загружаем изображение по URL
      const image = await ctx.api.uploadImage({ 
        url: WELCOME_IMAGE_URL 
      });

      // Отправляем ОДНО сообщение с изображением, текстом и клавиатурой
      await ctx.reply(message, {
        format: 'markdown',
        attachments: [image.toJson(), getAbiturientMenu()]
      });

    } catch (error) {
      console.error('❌ Ошибка загрузки изображения:', error);
      
      // Fallback: отправляем только текст и клавиатуру
      await ctx.reply(message, {
        format: 'markdown',
        attachments: [getAbiturientMenu()]
      });
    }
  });

  // Команда помощи
  bot.command('help', async (ctx) => {
    await ctx.reply(
      `🆘 **Помощь по боту**\n\n` +
      `Используйте кнопки меню для навигации.\n` +
      `Команда /start - перезапустить бота.`,
      {
        format: 'markdown',
        attachments: [getAbiturientMenu()]
      }
    );

  // Обработка произвольных текстовых сообщений
  
  });

  bot.on('message_created', async (ctx) => {
    // Пропускаем сообщения, которые уже обработаны другими обработчиками
    if (ctx.message.body.text && !ctx.message.body.text.startsWith('/')) {
      await handleUnknownMessage(ctx);
    }
  });

  // Команда для быстрого просмотра дедлайнов
  bot.command('deadlines', async (ctx) => {
    const deadlinesKeyboard = Keyboard.inlineKeyboard([
      [Keyboard.button.callback('📅 Посмотреть дедлайны', 'deadlines:view')]
    ]);

    await ctx.reply('Перейдите к просмотру дедлайнов:', {
      attachments: [deadlinesKeyboard]
    });
  });

  // Контакты
  bot.action('contacts:view', async (ctx) => {
    try {
      const location = new LocationAttachment({ 
        lat: 43.117778, 
        lon: 131.893056
      });

      await ctx.reply(
        `**📞 Контакты приемной комиссии**\n\n` +
        `📍 Адрес: Кампус ДВФУ, ГР-3, Владивосток, Приморский край\n` +
        `📞 Телефон: 8-800-550-38-38\n` +
        `✉️ Email: pochta_priemki.ru\n\n` +
        `🕒 **Часы работы:**\n` +
        `Пн-Пт: 9:00-18:00\n` +
        `Сб: 10:00-16:00\n` +
        `Вс: выходной`,
        {
          format: 'markdown',
          attachments: [location.toJson(), getAbiturientMenu()]
        }
      );
    } catch (error) {
      console.error('❌ Ошибка в contacts:view:', error);
      await ctx.reply('Информация о контактах временно недоступна.');
    }
  });

  // Документы
  bot.action('documents:list', async (ctx) => {
    try {
      const documents = [
        "📄 Паспорт (оригинал + копия)",
        "📄 Аттестат (оригинал + копия)", 
        "📄 4 фотографии 3x4 см",
        "📄 Медицинская справка 086/у",
        "📄 Документы о льготах (при наличии)",
        "📄 СНИЛС (копия)"
      ];

      await ctx.reply(
        "Необходимые документы:\n\n" +
        documents.join('\n') +
        "\n\n📝 Заявление заполняется в приемной комиссии",
        {
          format: 'markdown',
          attachments: [getAbiturientMenu()]
        } +
        documents.join('\n') +
        'Или вы можете подать заявление через сайт Госуслуги'
        
      );
    } catch (error) {
      console.error('❌ Ошибка в documents:list:', error);
      await ctx.reply('Информация о документах временно недоступна.');
    }
  });

  // Возврат в главное меню
  bot.action('menu:main', async (ctx) => {
    await ctx.reply('Главное меню:', {
      attachments: [getAbiturientMenu()]
    });
  });

  async function handleUnknownMessage(ctx) {
  const userMessage = ctx.message.body.text.toLowerCase();
  
  // Проверяем, похоже ли сообщение на вопрос
  const isQuestion = /^(что|как|где|когда|почему|зачем|кто|куда)\s+|[\?]/.test(userMessage);
  
  let response = '';

  if (isQuestion) {
    response = `Похоже, у вас вопрос!\n\n` +
      `К сожалению, я пока не умею отвечать на произвольные вопросы. ` +
      `Но я могу помочь вам с:\n\n` +
      `• 📋 Частыми вопросами о поступлении\n` +
      `• 🎓 Информацией о направлениях подготовки\n` +
      `• 🧮 Расчетом шансов поступления\n` +
      `• 📅 Дедлайнами подачи документов\n\n` +
      `Выберите нужный раздел в меню ниже ↓`;
  } else {
    response = `🤖 бот-помощник для абитуриентов ДВФУ\n\n` +
      `Я не совсем понял, что вы имеете в виду. ` +
      `Но я могу помочь вам с поступлением!\n\n` +
      `Вот что я умею:\n` +
      `• Отвечать на частые вопросы о поступлении\n` +
      `• Показывать направления подготовки\n` +
      `• Рассчитывать шансы поступления\n` +
      `• Напоминать о дедлайнах\n\n` +
      `Просто нажмите на кнопку в меню↓\n`;
  }

  await ctx.reply(response, {
    format: 'markdown',
    attachments: [getAbiturientMenu()]
  });
}
}